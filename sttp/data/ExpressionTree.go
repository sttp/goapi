...
func (et *ExpressionTree) lessThanOp(leftValue, rightValue *ValueExpression, valueType ExpressionValueTypeEnum) (*ValueExpression, error) {
	...
	if err := convertOperands(&leftValue, &rightValue, valueType); err != nil {
		return nil, errors.New("less than \"<\" operator " + err.Error())
	}
	...
	case ExpressionValueType.DateTime:
		return newValueExpression(ExpressionValueType.Boolean, leftValue.dateTimeValue().Before(rightValue.dateTimeValue())), nil
	case ExpressionValueType.Undefined:
		return nil, errors.New("cannot apply less than \"<\" operator to \"" + valueType.String() + "\"")
	default:
		return nil, errors.New("unexpected expression value type encountered")
	}
}
